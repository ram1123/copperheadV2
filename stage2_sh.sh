...
if [[ "$step" == "2" || "$step" == "all" ]]; then
    ...
    save_path="output/bdt_${model_name}_${model_trainYear}/ggH/score_edge_generation/"
...
if [[ "$step" == "5" || "$step" == "all" ]]; then
    ...
    print_box "Step:5:  Fig 6.19 (Run with ENV: source setup_env.sh)"
    ...
    # # ERROR: 
    print_box "Step:5:  Table 6.2 and 6.12"
