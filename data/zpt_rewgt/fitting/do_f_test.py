...
def perform_f_test(hist_SF, fit_xmin, fit_xmax, target_nbins, outTextFile, outTextFile_keys, year, njet):
    ...
    for order in range(2, 8):
        ...
        fit_func_low = ROOT.TF1(f"poly{order}", polynomial_expr, 0, fit_xmax)
        ...
        # Define the TF1 function with the generated expression
        fit_func_high = ROOT.TF1(f"poly{order}", polynomial_expr, 0, fit_xmax)
...
for njet in args.njet:
    ...
    # input_file = f"{save_path}/{year}_njet{njet}.root"
    # file = ROOT.TFile(f"{year}_njet{njet}.root", "READ")
    # input_file = f"{save_path}/{year}_njet{njet}_nbins{args.nbins}.root"
    ...
    for target_nbins in [args.nbins]: # INFO: What is the purpose of this target_nbins?
        ...
        canvas.SaveAs(f"{save_path}/SF_{year}_njet{njet}_{target_nbins}.pdf")
