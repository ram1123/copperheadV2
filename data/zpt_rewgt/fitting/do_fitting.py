...
if __name__ == "__main__":
    ...
    for year in args.years:
        ...
        for njet in args.njet:
            ...
            binning_array = np.linspace(0, 200, args.nbins)
            ...
            output_file = ROOT.TFile(f"{outputDirectory}/{year}_njet{njet}.root", "RECREATE")
            workspace.Write()
            output_file.Close()
            ...
            canvas.SaveAs(f"{outputDirectory}/dataDy_{year}_njet{njet}.png")
            canvas.SetLogy(1)
            canvas.SaveAs(f"{outputDirectory}/dataDy_{year}_njet{njet}_log.png")

            # Plot SF histogram
            canvas.Clear()
            canvas.SetLogy(0)
            hist_SF.Draw()
            canvas.SaveAs(f"{outputDirectory}/SF_{year}_njet{njet}.png")
