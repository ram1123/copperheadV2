...
for year in "${years[@]}"; do
    ...
    case "$mode" in
        ...
        2p)
            ...
            mva_name_suffix="_${save_postfix}_NoSyst"
            # mva_name_suffix="_${save_postfix}"
            load_path="${save_path}/stage2_histograms/score_${label}${mva_name_suffix}"
            # load_path="${save_path}/stage2_histograms/score_Run2_NanoV12_forVBFChannel_Apr29_2026_May06_2026_NoSyst"
            echo ${load_path}
        ...
        all)
            ...
            log "Command: $command0"
            eval "$command0"
            log "Runnings stage1 for year $year..."
            log "Command: $command1"
            eval "$command1"
            ;;
        zpt_fit|zpt_fit0|zpt_fit1|zpt_fit2|zpt_fit12)
            log "Running ZpT fitting step(s)..."
            dy_sample="aMCatNLO" # FIXME: Hardcoded DY sample name: aMCatNLO or MiNNLO or amcatnloFXFX or powheg or INCamcatnloFXFX
            # dy_sample="MiNNLO" # FIXME: Hardcoded DY sample name: aMCatNLO or MiNNLO or amcatnloFXFX or powheg or INCamcatnloFXFX
