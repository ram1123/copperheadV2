...
def process_file(remote_file):
    ...
    try:
        result = subprocess.run(
            ["python", "/depot/cms/private/users/shar1172/copperheadV2_CheckSetup/scripts/process_root_file.py", remote_file],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
        )
        return (remote_file, True, result.stdout.strip())
