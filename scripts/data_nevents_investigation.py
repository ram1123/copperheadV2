...
from dask import delayed, compute  # Import dask.delayed and dask.compute
...
def process_partition(partition):
    """
    Process a partition in a Dask DataFrame.
    Add the 'outputNanoAODFile' column after splitting the inputMiniAOD column.
    """
    partition['outputNanoAODFile'] = partition['inputMiniAOD'].fillna('').apply(
        lambda x: x.split('/')[-1].replace(".root", "_NanoAOD.root") if isinstance(x, str) else ""
    )
    return partition
