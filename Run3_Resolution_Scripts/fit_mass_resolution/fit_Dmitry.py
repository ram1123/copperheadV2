...
if config["plot_types"]["fit_signal"] or config["plot_types"]["fit_z_peak"]:
    # If fitting, we need to load the dimuon mass and nominal weight only
...
all_vars = sublead_vars + dimuon_vars

comparer = DistributionCompare(year, input_paths_labels, fields_to_load, control_region, directoryTag, "config/varlist.yaml")
comparer.load_data()
if not (config["plot_types"]["fit_signal"] or config["plot_types"]["fit_z_peak"]):
    # These additional variables are not needed for the fit
...
if config["plot_types"]["fit_z_peak"] and control_region in ["z-peak", "z_peak"]:
    ...
    # Plot in double-muon regions (eta1 x eta2)
    if config["fit_categories"]["z-peak"]["regions"]:
        for region in ["BB", "BO", "BE", "OB", "OO", "OE", "EB", "EO", "EE"]:
            ...
            if config["fit_categories"]["z-peak"]["binned"]:
                comparer.fit_dimuonInvariantMass_DCBXBW(suffix=region, events_dict=filtered)
            if config["fit_categories"]["z-peak"]["unbinned"]:
                comparer.fit_dimuonInvariantMass_DCBXBW_Unbinned(suffix=region, events_dict=filtered)


if config["plot_types"]["fit_signal"] and control_region == "signal":
    if config["fit_categories"]["signal"]["binned"]:
        comparer.fit_dimuonInvariantMass_DCB(suffix="Inclusive")
    if config["fit_categories"]["signal"]["unbinned"]:
        comparer.fit_dimuonInvariantMass_DCB_Unbinned(suffix="Inclusive")
    ...
    for region in ["BB", "BO", "BE", "OB", "OO", "OE", "EB", "EO", "EE"]:
        ...
        filtered = {label: comparer.filter_eta(events, region) for label, events in comparer.events.items()}
        if config["fit_categories"]["signal"]["binned"]:
            comparer.fit_dimuonInvariantMass_DCB(events_dict=filtered, suffix=region)
        if config["fit_categories"]["signal"]["unbinned"]:
