...
def filterRegion_UsingRapidity(events, region):
    ...
    if region == "BB":
        events_cuts = (events.mu1_eta <= 0.9) & (events.mu2_eta <= 0.9)
    elif region == "BO":
        events_cuts = (events.mu1_eta <= 0.9) & (events.mu2_eta > 0.9) & (events.mu2_eta <= 1.8)
    elif region == "OB":
        events_cuts = (events.mu1_eta > 0.9) & (events.mu1_eta <= 1.8) & (events.mu2_eta <= 0.9)
    elif region == "OO":
        events_cuts = (events.mu1_eta > 0.9) & (events.mu1_eta <= 1.8) & (events.mu2_eta > 0.9) & (events.mu2_eta <= 1.8)
    elif region == "BE":
        events_cuts = (events.mu1_eta <= 0.9) & (events.mu2_eta > 1.8) & (events.mu2_eta <= 2.4)
    elif region == "EB":
        events_cuts = (events.mu1_eta > 1.8) & (events.mu1_eta <= 2.4) & (events.mu2_eta <= 0.9)
    elif region == "EO":
        events_cuts = (events.mu1_eta > 1.8) & (events.mu1_eta <= 2.4) & (events.mu2_eta > 0.9) & (events.mu2_eta <= 1.8)
    elif region == "OE":
        events_cuts = (events.mu1_eta > 0.9) & (events.mu1_eta <= 1.8) & (events.mu2_eta > 1.8) & (events.mu2_eta <= 2.4)
    elif region == "EE":
        events_cuts = (events.mu1_eta > 1.8) & (events.mu1_eta <= 2.4) & (events.mu2_eta > 1.8) & (events.mu2_eta <= 2.4)

    return events[events_cuts]
...
def fitPlot_ggh(events_bsOn, events_bsOff, save_filename, save_plot=True, selection = "", region=""):
    """
    ...
    if selection != "":
        # selection = f"dimuon_eta > 0 & dimuon_eta < 0.9" if region == "BB" else f"dimuon_eta > 0.9 & dimuon_eta < 1.5" if region == "BE" else f"dimuon_eta > 1.5 & dimuon_eta < 2.4"
        # print(f"selection: {selection}")

        dimuon_eta_bsOn = events_bsOn.dimuon_eta
        dimuon_eta_bsOff = events_bsOff.dimuon_eta

        if selection == "BB":
            selection_bsOn = (dimuon_eta_bsOn > 0) & (dimuon_eta_bsOn < 0.9)
            selection_bsOff = (dimuon_eta_bsOff > 0) & (dimuon_eta_bsOff < 0.9)
        elif selection == "BE":
            selection_bsOn = (dimuon_eta_bsOn > 0.9) & (dimuon_eta_bsOn < 1.5)
            selection_bsOff = (dimuon_eta_bsOff > 0.9) & (dimuon_eta_bsOff < 1.5)
        elif selection == "EE":
            selection_bsOn = (dimuon_eta_bsOn > 1.5) & (dimuon_eta_bsOn < 2.4)
            selection_bsOff = (dimuon_eta_bsOff > 1.5) & (dimuon_eta_bsOff < 2.4)

        events_bsOn = events_bsOn[selection_bsOn]
...
if __name__ == "__main__":
    ...
    # for region in ["BB", "BE", "EE"]:
        # fitPlot_ggh(events_BSon, events_BSoff, f"BSC_geofit_comparison_2022PreEE_{region}.pdf", save_plot=True, selection=region)
