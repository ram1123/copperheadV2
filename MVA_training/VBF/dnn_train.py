...
def safe_atanh(x):
    x = np.clip(x, -1 + _ATANH_EPS, 1 - _ATANH_EPS)
    out = np.arctanh(x)
    return np.where(np.isfinite(out), out, 0.0)


def torch_safe_atanh(x):
    x = torch.clamp(x, -1 + _ATANH_EPS, 1 - _ATANH_EPS)
    out = 0.5 * (torch.log1p(x) - torch.log1p(-x))  # atanh via logs
...
def transformDnnScore(dnn_scores):
    s = np.clip(dnn_scores, 1e-6, 1-1e-6) # protection from arctanh(0) or arctanh(1 or -1) whose value is +/- inf
    return np.arctanh(s)
...
def plotSigVsBkg(score_dict, bins, plt_save_path, transformPrediction=False, normalize=True, log_scale=False):
    ...
    for stage, output_dict in score_dict.items():
        ...
        if transformPrediction:
            pred_total = safe_atanh(pred_total)
            if log_scale:
                plt.ylim((0.0001, 1e5))
            else:
                plt.ylim((0.0, 5.0))
        ...
        plt.ylim((0.0001, max_bin_content * 1.1))
...
def dnn_train(model, data_dict, fold_idx, training_features, batch_size, nepochs, save_path,
    ...
    dataloader_train_ordered = DataLoader(dataset_train, batch_size=batch_size, shuffle=True,  pin_memory=PIN_MEMORY) # for plotting
    dataset_valid = NumpyDataset(input_arr_valid, label_arr_valid)
    dataloader_valid = DataLoader(dataset_valid, batch_size=batch_size, shuffle=True,  pin_memory=PIN_MEMORY)
    dataset_eval = NumpyDataset(input_arr_eval, label_arr_eval)
    dataloader_eval = DataLoader(dataset_eval, batch_size=batch_size, shuffle=True,  pin_memory=PIN_MEMORY)
    best_significance = 0
    ...
    for epoch in range(nepochs):
        ...
        dataloader_train = DataLoader(dataset_train, batch_size=batch_size, shuffle=True,  pin_memory=PIN_MEMORY)
...
def main():
    ...
    best_hp = {
        ...
        "batch_size": 10048, #args.batch_size,
        "loss_name": "bce",
    }
