...
class Net(nn.Module):
    ...
    def forward(self, x):
        x = self.dropout1(self.bn1(self.act(self.fc1(x))))
        x = self.dropout2(self.bn2(self.act(self.fc2(x))))
        x = self.dropout3(self.bn3(self.act(self.fc3(x))))
...
def dnn_train(model, data_dict, fold_idx, training_features, batch_size, nepochs, save_path, callback=None,
              lr=1e-3, optimizer_name="adam", weight_decay=0.0, loss_name="bce"
              ):
...
def _make_loaders_from_df(df_train, df_valid, training_features, batch_size):
    x_tr = df_train[training_features].values; y_tr = df_train.label.values
    x_va = df_valid[training_features].values; y_va = df_valid.label.values
    ds_tr = NumpyDataset(x_tr, y_tr); ds_va = NumpyDataset(x_va, y_va)
    dl_tr = DataLoader(ds_tr, batch_size=batch_size, shuffle=True,  num_workers=NWORKERS, pin_memory=PIN_MEMORY)
    dl_va = DataLoader(ds_va, batch_size=batch_size, shuffle=False, num_workers=NWORKERS, pin_memory=PIN_MEMORY)
    return dl_tr, dl_va
...
def _valid_auc(model, dl, device):
    ...
    for xb, yb in dl:
        ...
        probs.append(p.ravel()); labels.append(yb.numpy().ravel())
    return roc_auc_score(np.concatenate(labels), np.concatenate(probs))

def run_bayes_opt(
    fold_for_bo_path, training_features, bo_trials=60, bo_init=15, bo_epochs=30, device=None
):
    """
    Bayesian optimization on ONE fold (fast). Returns a dict of best params.
    """
    device = device or (torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu"))

    # Load the chosen fold once
    df_train = pd.read_parquet(f"{fold_for_bo_path}/data_df_train.parquet")
    df_valid = pd.read_parquet(f"{fold_for_bo_path}/data_df_validation.parquet")

    # Ax search space
    gs = GenerationStrategy(steps=[
        GenerationStep(Models.SOBOL, num_trials=bo_init, min_trials_observed=bo_init),
        GenerationStep(Models.GPEI,  num_trials=-1),  # switch to batch QNEI if you want parallel BO
    ])
    ax = AxClient(generation_strategy=gs)
    ax.create_experiment(
        name="hmm_dnn_bo",
        parameters=[
            {"name":"hidden0",      "type":"choice", "values":[64,128,256,512]},
    ...
    # Precompute feature names with variation handling
    tr_features = prepare_features(df_train, training_features)

    for _ in range(bo_trials):
        p, trial_idx = ax.get_next_trial()

        # derive layer sizes from (hidden0, shrink1, shrink2)
        h1 = int(p["hidden0"])
        h2 = max(8, int(h1 * float(p["shrink1"])))
        h3 = max(8, int(h2 * float(p["shrink2"])))
        d  = float(p["dropout"])

        # data loaders for this batch size
        dl_tr, dl_va = _make_loaders_from_df(df_train, df_valid, tr_features, int(p["batch_size"]))

        # model
        model = Net(
            n_feat=len(tr_features),
            hidden=(h1, h2, h3),
            dropout=(d, d, d),
        ...
        # loss
        loss_fn = FocalLoss(alpha=1, gamma=2) if p["loss_name"] == "focal" else torch.nn.BCELoss()

        # optimizer
        if p["optimizer"] == "adamw":
            optim_ = optim.AdamW(model.parameters(), lr=float(p["lr"]), weight_decay=float(p["weight_decay"]))
        else:
            optim_ = optim.Adam(model.parameters(),  lr=float(p["lr"]), weight_decay=float(p["weight_decay"]))

        # train few epochs (fast) with early stop
        best_auc, bad, patience = -1.0, 0, 5
        for _epoch in range(bo_epochs):
            model.train()
            for xb, yb in dl_tr:
                xb = xb.to(device); yb = yb.to(device).reshape((-1,1))
                optim_.zero_grad()
        ...
        ax.complete_trial(trial_idx, raw_data=best_auc)

    best_params, best_vals = ax.get_best_parameters()
    # derive final layer sizes to feed main training
    h1 = int(best_params["hidden0"])
    h2 = max(8, int(h1 * float(best_params["shrink1"])))
    h3 = max(8, int(h2 * float(best_params["shrink2"])))
    best_params["hidden"]  = (h1, h2, h3)
    best_params["dropout"] = (float(best_params["dropout"]),) * 3
    return best_params, best_vals["auc"]["mean"]
...
def main():
    ...
    parser.add_argument(
        "--bo",
        action="store_true",
        help="Run Bayesian optimization before training")
    parser.add_argument(
        "--bo-trials",
        type=int,
        default=60)
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Which fold to use for BO evaluation")
    ...
    if args.bo:
        # point the BO helper at a single fold’s parquet (reuse the same path layout)
        bo_fold = int(args.bo_fold)
        fold_for_bo_path = f"{save_path}"  # files are already per-fold with suffixes
        # For BO, we expect: data_df_train.parquet, data_df_validation.parquet
        # If your files are suffixed per fold, write tiny symlinks or just load fold bo_fold:
        ...
        # create temporary, unsuffixed copies for the helper
        df_tr.to_parquet(f"{save_path}/data_df_train.parquet")
        df_va.to_parquet(f"{save_path}/data_df_validation.parquet")

        best_hp, best_auc = run_bayes_opt(
            fold_for_bo_path=save_path,
            training_features=training_features,
            bo_trials=args.bo_trials,
            bo_init=args.bo_init,
            bo_epochs=args.bo_epochs,
        )
        logger.info(f"[BO] Best AUC={best_auc:.5f}")
        logger.info(f"[BO] Best params={best_hp}")
    ...
    for i in range(nfolds):
        ...
        batch_sz  = int(best_hp.get("batch_size", args.batch_size))
