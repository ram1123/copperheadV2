...
ARRAY_TASKS="${BIAS_ARRAY_TASKS:-100}"
...
if [[ "${RUN_MODE}" == "local" ]]; then
    ...
    echo "Local array-task replicas per (truth, fit) pair: ${ARRAY_TASKS}"
    job_id_base="$(date +%s)"
    running_jobs=0
    task_id=1
    ...
    for in_index in "${truth_indices[@]}"; do
        for out_index in {1..1}; do # set core pdf index to BWZ redux, but it is NOT frozen
            for array_task_id in $(seq 1 "${ARRAY_TASKS}"); do
                ...
                pixi run -e combine --manifest-path /cvmfs/cms-af.opensciencegrid.org/paf/pixi/copperheadV2_dev/pixi.toml \
                    sh "${SCRIPT_DIR}/slurm_combine_sh.sh" "${array_task_id}" "${local_job_id}" "${in_index}" "${out_index}" "${WORK_DIR}" &
