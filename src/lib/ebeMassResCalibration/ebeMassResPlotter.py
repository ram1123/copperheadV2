...
def generateVoigtian_plot(mass_arr, cat_idx: int, nbins=100, df_fit=""):
    """
    params
...
def generateBWxDCB_plot(mass_arr, cat_idx: int, nbins=100, df_fit=""):
    """
    params
    mass_arr: numpy arrary of dimuon mass value to do calibration fit on
    cat_idx: int index of specific calibration category the mass_arr is from
    ...
    # n2 = rt.RooRealVar("n2" , "n2", 114, 0.01, 385) #test
    n1.setConstant(True)
    n2.setConstant(True)
...
if __name__ == "__main__":
    ...
    # for idx in range(0, 12):
        ...
        df_fit = generateBWxDCB_plot(cat_dimuon_mass, idx, nbins=nbins, df_fit=df_fit)
        # if idx < 12:
        #     df_fit = generateBWxDCB_plot(cat_dimuon_mass, idx, nbins=nbins, df_fit=df_fit)
        # else:
        #     df_fit = generateVoigtian_plot(cat_dimuon_mass, idx, nbins=nbins, df_fit=df_fit)

    df_fit.to_csv(f"calib_fit_results_OLD_CODE.csv", index=False)
